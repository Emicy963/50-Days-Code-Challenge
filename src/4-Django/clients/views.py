...
def pedidos_pdf_report(request):
    ...
    if data_inicio:
        try:
            data_inicio_parsed = datetime.datetime.strptime(data_inicio, '%Y-%m-%d').date()
            pedidos = pedidos.filter(data_pedido__date__gte=data_inicio_parsed)
        except ValueError:
            pass
    ...
    # Criar o PDF
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="relatorio_pedidos_{datetime.date.today()}.pdf"'
    ...
    if prioridade:
        filter_info.append(f"Prioridade: {dict(Pedido.PRIORIDADE_CHOICES).get(prioridade, prioridade)}")
    ...
    if pedidos_lista:
        ...
        for pedido in pedidos_lista:
            pedidos_data.append([
                pedido.numero_pedido,
                pedido.cliente.name[:20] + '...' if len(pedido.cliente.name) > 20 else pedido.cliente.name,
                pedido.data_pedido.strftime('%d/%m/%Y'),
                pedido.get_status_display(),
        ...
        pedidos_table = Table(pedidos_data, colWidths=[1*inch, 1.5*inch, 1*inch, 1*inch, 1*inch, 1*inch])
        pedidos_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
...
def export_pedidos_csv(request):
    ...
    # Criar resposta CSV
    response = HttpResponse(content_type='text/csv; charset=utf-8')
    response['Content-Disposition'] = f'attachment; filename="pedidos_{datetime.date.today()}.csv"'
    ...
    for pedido in pedidos_list:
        writer.writerow([
            ...
            pedido.data_entrega_prevista.strftime('%d/%m/%Y') if pedido.data_entrega_prevista else 'N/A',
            pedido.get_status_display(),
            pedido.get_prioridade_display(),
