...
urlpatterns = [
    ...
    path(
        ...
        views.update_pedido_status,
        name="update_pedido_status",
    ),
    path("pedidos/<int:id>/cancelar/", views.cancel_pedido, name="cancel_pedido"),
    path(
        "pedidos/acoes-lote/", views.bulk_actions_pedidos, name="bulk_actions_pedidos"
    ),
    path("pedidos/dashboard/", views.dashboard_pedidos, name="dashboard_pedidos"),
    # URLs de Integração Cliente-Pedidos
    path(
        "clientes/<int:client_id>/pedidos/", views.client_pedidos, name="client_pedidos"
    ),
    path(
        "clientes/<int:client_id>/pedidos/criar/",
        views.create_pedido_for_client,
        name="create_pedido_for_client",
    ),
    # URLs para relatórios PDF
    path(
        "cliente/<int:client_id>/pdf/",
        views.client_pdf_report,
        name="client_pdf_report",
    ),
    ...
    path(
        "dashboard/export-csv/", views.export_dashboard_csv, name="export_dashboard_csv"
    ),
    # URLs AJAX
    path("ajax/buscar-cep/", views.get_address_by_cep, name="buscar_cep"),
    path(
        "ajax/pedidos/<int:id>/status/",
        views.ajax_update_pedido_status,
        name="ajax_update_pedido_status",
    ),
    path(
        "ajax/clientes/<int:client_id>/pedidos/",
        views.ajax_get_client_pedidos,
        name="ajax_get_client_pedidos",
    ),
]
